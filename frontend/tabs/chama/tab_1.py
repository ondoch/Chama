...
class Tab1(QFrame):
    ...
    def initUI(self):
        ...
        chama_name = FormInput("resources/user.svg", "Chama Name")
        description = FormInput("resources/description.svg", "Description of the chama")
        contribution = FormInput("resources/contribution.svg", "Amount of contribution")
        pool_percentage = FormInput("resources/percent.svg", "Pool percentage e.g 10%")

        registration_number = GenerateFormInput("resources/certificate.svg", "Chama registration number")
        meeting_frequency = FormInput("resources/calender.svg", "Meeting frequency")
        share_percentage = FormInput("resources/percent.svg", "Chama share percentage distribution e.g 10%")
        loan_percentage = FormInput("resources/percent.svg", "Loan percentage e.g 10%")

        row_1.addWidget(chama_name)
        row_1.addWidget(description)
        row_1.addWidget(contribution)
        row_1.addWidget(pool_percentage)

        row_2.addWidget(registration_number)
        row_2.addWidget(meeting_frequency)
        row_2.addWidget(share_percentage)
        row_2.addWidget(loan_percentage)
