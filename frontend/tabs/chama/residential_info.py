...
class ResidentialInfo(QFrame):
    ...
    def initUI(self):
        ...
        country = MemberFormInput("Country of residence")
        county = MemberFormInput("County of residence")
        town = MemberFormInput("Town/City of residence")
        estate = MemberFormInput("Estate/Address")

        physical_address = MemberFormInput("Physical Address")
        postal_address = MemberFormInput("Postal Address")
        postal_code = MemberFormInput("Postal Code")

        grid.addWidget(country, 0, 0)
        grid.addWidget(county, 1, 0)
        grid.addWidget(town, 2, 0)
        grid.addWidget(estate, 3, 0)

        grid.addWidget(physical_address, 0, 1)
        grid.addWidget(postal_address, 1, 1)
        grid.addWidget(postal_code, 2, 1)
